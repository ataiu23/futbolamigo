...
class UltimaokjaviSupabaseUser extends BaseAuthUser {
  UltimaokjaviSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> ultimaokjaviSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = UltimaokjaviSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
